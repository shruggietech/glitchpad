...
Android provider lifecycle work requires headless x86_64 API 24 and API 36 emulators. From `crates/glitchpad-host/gen/android`, run `gradlew :glitchpad-android-source:testDebugUnitTest` for Kotlin policy tests and `gradlew :app:connectedUniversalDebugAndroidTest -PabiList=x86_64 -ParchList=x86_64 -PtargetList=x86_64 -x :app:rustBuildUniversalDebug -x :app:rustBuildX86_64Debug` after the x86_64 Tauri build for the controlled DocumentsProvider suite. CI additionally runs restoration seed and verify phases around `adb shell am force-stop com.shruggietech.glitchpad`; local evidence is optional when a compatible headless emulator is unavailable, but both CI matrix entries remain required for merge.
