...
import { history, historyKeymap, indentLess, indentMore, indentWithTab, redo, undo } from '@codemirror/commands';
import { bracketMatching, defaultHighlightStyle, indentUnit, syntaxHighlighting } from '@codemirror/language';
import { closeSearchPanel, findNext, findPrevious, gotoLine, openSearchPanel, searchKeymap } from '@codemirror/search';
...
import type { LanguageDecision, LanguageId, ShellSession, TextDocumentState } from '../domain/contracts';
import { detectLanguage, LanguageLoader } from '../domain/language';
import { applyTextTransaction } from '../domain/text-document';
...
export const TextEditorSurface = forwardRef<TextEditorHandle, TextEditorSurfaceProps>(
  function TextEditorSurface({ session, onDocumentChange, onLanguageChange }, handleRef) {
    ...
    const [languageStatus, setLanguageStatus] = useState(session.text_document?.language.status ?? 'plain');
    ...
    useImperativeHandle(handleRef, () => ({
      invoke(command) {
        ...
        if (command === 'toggle_wrap') {
          ...
          view.dispatch({ effects: wrappingCompartment.current.reconfigure(wrappedRef.current ? EditorView.lineWrapping : []) });
          return true;
        }
        if (command === 'copy') {
          const ranges = view.state.selection.ranges.filter(({ empty }) => !empty);
          const content = ranges.length > 0
            ? ranges.map(({ from, to }) => view.state.sliceDoc(from, to)).join('\n')
    ...
    useEffect(() => {
      ...
      const canEdit = textDocument.mode === 'editable' && session.renderer.capabilities.edit && session.source.capabilities.write;
      ...
      const view = new EditorView({
        ...
        state: EditorState.create({
          ...
          extensions: [
            ...
            keymap.of([indentWithTab, ...closeBracketsKeymap, ...historyKeymap, ...searchKeymap]),
            indentUnit.of('  '),
            ...
            EditorView.updateListener.of((update) => {
              ...
              if (!result.ok) return;
              const nextDocument = result.document.language.origin === 'automatic'
                ? {
                    ...result.document,
                    language: {
                      ...detectLanguage(session.source.display_name, result.document.normalized_text),
              ...
              callbackRef.current(session.id, expectedRevision, nextDocument, result.revision);
            }),
            EditorView.theme({
              '&': { height: '100%', backgroundColor: 'transparent' },
              '.cm-scroller': { fontFamily: 'ui-monospace, Cascadia Code, Consolas, monospace' },
              '.cm-content': { caretColor: 'var(--focus-color)' },
              '.cm-gutters': { backgroundColor: 'transparent', borderRightColor: '#343842' },
            }),
          ],
        }),
      });
    ...
    useEffect(() => {
      ...
      void loaderRef.current.load(textDocument.language.language, textDocument.language.load_revision).then((result) => {
        if (viewRef.current !== view) return;
        setLanguageStatus(result.status);
        view.dispatch({ effects: languageCompartment.current.reconfigure(result.support ?? []) });
      });
    }, [session.id, session.text_document?.language.language, session.text_document?.language.load_revision, session.text_document?.longest_line_bytes]);
    ...
    return (
      <div className="text-editor-shell">
        <div className="editor-status" aria-label="Text document status">
          <span>{session.text_document?.mode === 'editable' && session.renderer.capabilities.edit && session.source.capabilities.write ? 'Editable' : 'Read only'}</span>
          <span>{profile?.encoding.replaceAll('_', ' ')}</span>
          <span>{profile?.bom === 'present' ? 'BOM' : 'No BOM'}</span>
          <span>{profile?.newline_pattern}</span>
          <span>{profile?.terminal_newline ? 'Terminal newline' : 'No terminal newline'}</span>
          <span>{profile?.round_trip_safe ? 'Round-trip safe' : 'Save decision required'}</span>
          <span>{languageStatus}</span>
          <label>
            Language
            <select
              aria-label="Language mode"
              value={session.text_document?.language.origin === 'session_override' ? session.text_document.language.language : 'automatic'}
              onChange={(event) => {
                const value = event.target.value;
                const current = session.text_document;
                if (!current) return;
                const language = value === 'automatic'
                  ? detectLanguage(session.source.display_name, current.normalized_text)
                  : {
                      ...current.language,
                      language: value as LanguageId,
                      origin: 'session_override' as const,
                      status: value === 'plain_text' ? 'plain' as const : 'loading' as const,
            ...
            >
              ...
              {['plain_text', 'rust', 'typescript', 'javascript', 'python', 'json', 'toml', 'yaml', 'css', 'html'].map((language) => (
                <option key={language} value={language}>{language.replaceAll('_', ' ')}</option>
              ))}
            </select>
          </label>
          <span>{session.text_document?.source_bytes.toLocaleString()} bytes</span>
        </div>
