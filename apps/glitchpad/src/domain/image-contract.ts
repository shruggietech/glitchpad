...
export type ImageFamilyState =
  ...
  | { family: 'ico'; entries: { index: number; width: number; height: number; bits_per_pixel: number; encoded_bytes: number; preview: 'full' | 'thumbnail' | 'unavailable' | 'refused'; encoding: 'png' | 'dib'; alpha: boolean | null; failure: string | null; duplicate_of: number | null }[]; selected_entry: number | null; selected_entry_export: boolean };
...
const validateFamilyState = (state: unknown): state is ImageFamilyState => {
  ...
  return state.entries.every((entry: unknown, index: number) => record(entry) && entry.index === index && count(entry.width, 256) && Number(entry.width) > 0 && count(entry.height, 256) && Number(entry.height) > 0 && count(entry.bits_per_pixel, 65535) && count(entry.encoded_bytes, 128 * 1024 * 1024) && ['png', 'dib'].includes(String(entry.encoding)) && ['full', 'thumbnail', 'unavailable', 'refused'].includes(String(entry.preview)) && (entry.alpha === null || typeof entry.alpha === 'boolean') && (entry.failure === null || token(entry.failure)) && (entry.duplicate_of === null || count(entry.duplicate_of, index - 1)));
};
