# Glitchpad Technical Specification v0.1.1
...
36. [Roadmap Beyond v0.1.1](#36-roadmap-beyond-v011)
37. [Open Questions](#37-open-questions)
38. [Appendices](#38-appendices)

## 1. Document Control and Authority

This document is the architecture, behavior, platform, security, development, and delivery contract for Glitchpad v0.1.1, the corrective successor to the first installable community release. Its version MUST equal the product version.
...
- v0.1.1 ships official Windows, macOS, Linux, and Android artifacts with Markdown, Mermaid, and text/source view/edit, tabs, metadata inspection, save conflict protection, and crash recovery.
...
v0.1.1 promotes the completed text-family capabilities to `stable`. A capability becomes `experimental` or `stable` only through a release that includes its implementation, platform artifacts, conformance evidence, associations, documentation, and notices.

| Format family | Examples | v0.1.1 status | First release | Capabilities | Fidelity boundary |
| --- | --- | --- | --- | --- | --- |
...
- Windows, macOS, Linux, and Android are foundational targets and release-blocking for v0.1.1.
...
Desktop uses one application window with one compact tab strip. Android uses one activity; phones show the active title and a tab-count switcher that opens a sheet, while tablets may show the desktop-style strip. v0.1.1 does not support multiple application windows.
...
All four target families are Tier 1 for v0.1.1. A Tier 1 platform requires an official artifact, native build host, automated shared suite, adapter tests, installation smoke tests, core user-flow evidence, version and license evidence, and a named support baseline.

| Platform | Runtime baseline | Architectures | Official v0.1.1 artifacts | Native WebView |
| --- | --- | --- | --- | --- |
...
Windows ARM64 and Linux ARM64 are planned platform expansions after v0.1.1 and require their own build, package, and device evidence. iOS is outside the current platform boundary.
...
The v0.1.1 preference schema contains theme, editor font family and size, line wrapping, tab width, Markdown default mode, and explicit language overrides. It contains no account, synchronization, workspace, telemetry, remote-resource, plugin, or recent-file setting.
...
- NSIS tooling selected by the Tauri lock. v0.1.1 is distributed as an unsigned community release.
...
- Standard Apple command-line packaging tools. v0.1.1 uses an ad-hoc application signature and is not notarized.
...
There are no unresolved architecture or product questions in v0.1.1. Implementation discoveries that materially challenge this specification MUST create a Spec Kit clarification or architecture-amendment slice before code selects a different behavior.
...
| Four Tier 1 v0.1.1 platform families                    | 24      |
