...
S022 produces a universal APK, an ARM64 APK, and an Android App Bundle for the v0.1.0 candidate. The universal and Play artifacts contain ARM64 and x86_64 application libraries; the smaller direct-install APK contains ARM64 only. All three share application identifier `com.shruggietech.glitchpad`, version `0.1.0`/code `1000`, minimum API 24, and target API 36.
...
Physical-device installation, TalkBack, touch, rotation, background/restore, low-memory, and provider interoperability are intentionally deferred until after v0.1.0 publication and do not block S022 or the release.
