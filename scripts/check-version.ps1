...
if ($cargoManifest -notmatch '(?m)^version = "0\.1\.3"$') {
    $failures.Add('Cargo workspace version authority is not 0.1.3.')
}

if ($technicalSpecification -notmatch '^# Glitchpad Technical Specification v0\.1\.3') {
    $failures.Add('Technical specification heading is not v0.1.3.')
}

if ($readme -notmatch 'version-0\.1\.3') {
    $failures.Add('README version badge is not 0.1.3.')
}

if ($androidManifest -notmatch 'versionName = tauriProperties\.getProperty\("tauri\.android\.versionName", "0\.1\.3"\)') {
    $failures.Add('Android fallback version is not 0.1.3.')
}
