...
export function validatePortableSmokeContract(lifecycleSource, workflowSource) {
  ...
  for (const requirement of ['[S030_ALPHA_RAW_SENTINEL]: https://example.invalid/alpha', '[S030_BETA_RAW_SENTINEL]: https://example.invalid/beta', 'S038_RECOVERY_RAW_SENTINEL', '| Alpha | Beta |', 'Footnote[^1]', 'flowchart TB', 'Wait-SafeMarkdownOutcome', 'Wait-WindowText', 'Wait-NamedButton', 'TryGetCurrentPattern', 'GetClickablePoint', 'GlitchpadNativeInput', 'SetForegroundWindow', 'mouse_event', 'Send-MarkdownDelivery', 'Exercise-MarkdownEditSavePreview', 'Exercise-MarkdownRecovery', 'markdown-failure-request.marker', 'markdown-failure-consumed.marker', 'ConvertFrom-Json -AsHashtable', "[string]$scaleAuthority['source_commit']", "Invoke-MenuCommand $Process 'Edit source'", "Invoke-MenuCommand $Process 'Preview'", "[System.Windows.Forms.SendKeys]::SendWait('^s')", 'saved durably.', 'DocumentSurface.test.tsx', 'MarkdownSurface.test.tsx'])
