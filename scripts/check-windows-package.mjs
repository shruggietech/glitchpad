...
export function validatePortableSmokeContract(lifecycleSource, workflowSource) {
  ...
  for (const requirement of ['[S030_ALPHA_RAW_SENTINEL]: https://example.invalid/alpha', '[S030_BETA_RAW_SENTINEL]: https://example.invalid/beta', 'S038_RECOVERY_RAW_SENTINEL', '| Alpha | Beta |', 'Footnote[^1]', 'flowchart TB', 'Wait-SafeMarkdownOutcome', 'Wait-WindowText', 'Wait-NamedButton', 'Wait-ActionableNamedElement', 'TreeWalker]::ControlViewWalker.GetParent', 'TryGetCurrentPattern', 'Invoke-AutomationElement', 'Click-AutomationElement', 'catch { }', 'GetClickablePoint', '$bounds = $Element.Current.BoundingRectangle', '$clickX = $bounds.X + ($bounds.Width / 2)', '$clickY = $bounds.Y + ($bounds.Height / 2)', '$item = Wait-ActionableNamedElement $Process $Name', 'Click-AutomationElement $Process $item $Name', 'GlitchpadNativeInput', 'SetForegroundWindow', 'mouse_event', 'Send-MarkdownDelivery', 'Exercise-MarkdownEditSavePreview', 'Exercise-MarkdownRecovery', 'markdown-failure-request.marker', 'markdown-failure-consumed.marker', 'System.Text.Json.JsonDocument', "GetProperty('source_commit').GetString()", "GetProperty('workflow_identity').GetString()", "GetProperty('candidate_manifest_sha256').GetString()", "GetProperty('evidence_authority').GetProperty('source_commit').GetString()", "GetProperty('content_free').GetBoolean()", 'GetProperty($property).GetString()', "Invoke-MenuCommand $Process 'Edit source'", "Invoke-MenuCommand $Process 'Preview'", "[System.Windows.Forms.SendKeys]::SendWait('^s')", 'saved durably.', 'DocumentSurface.test.tsx', 'MarkdownSurface.test.tsx'])
...
export async function checkWindowsConfiguration(
  ...
  if (
    !same(
      contract.artifacts.map(({ name }) => name),
      expectedNames,
    )
  )
  ...
  if (!same(associated, configured))
    fail('Tauri associations drift from capabilities.json');
  for (const association of tauri.bundle.fileAssociations) {
    for (const extension of association.ext) {
      for (const operation of ['BACKUP', 'RESTORE']) {
        ...
        if (!installerHooks.includes(invocation))
          fail(
            `installer hooks omit ${operation.toLowerCase()} for ${extension}`,
          );
      }
    }
  }
  ...
  return {
    capabilityCount: configured.length,
...
function validateWindowsEvidenceShape(evidence, contract, official) {
  ...
  if (
    /PRIVATE KEY|client[_-]?secret|certificate[_-]?password/iu.test(serialized)
  )
  ...
  const expected = new Map(
    contract.artifacts.map((artifact) => [artifact.kind, artifact.name]),
  );
  const seen = new Set();
  for (const artifact of evidence.artifacts) {
    if (
      seen.has(artifact.kind) ||
      expected.get(artifact.kind) !== artifact.name
    )
      fail(`unexpected or duplicate artifact ${artifact.kind}`);
    seen.add(artifact.kind);
    if (!sha256Pattern.test(artifact.sha256))
      fail(`invalid digest for ${artifact.kind}`);
    const classification = classifyPackageSize(
      artifact.bytes,
      contract.size_budget,
    );
    if (
      classification !== artifact.size_classification ||
      classification === 'failure'
    )
      fail(`invalid size result for ${artifact.kind}`);
    if (
      !official &&
      artifact.signature_status !== contract.candidate_signature_status
    )
      fail(`candidate ${artifact.kind} has an invalid signature state`);
  }

  if (!Array.isArray(evidence.portable_inventory))
    fail('portable inventory is missing');
  ...
  for (const entry of evidence.portable_inventory) {
    ...
    if (caseFolded.has(folded))
      fail(`case-colliding inventory path ${entry.relative_path}`);
    ...
    if (
      entry.relative_path.toLowerCase().endsWith('.exe') &&
      entry.relative_path !== 'Glitchpad.exe'
    )
      fail(`unexpected executable ${entry.relative_path}`);
  }
  for (const required of contract.portable_required_files)
    if (!inventoryNames.has(required))
      fail(`portable inventory omits ${required}`);
  ...
  if (
    !Array.isArray(evidence.evidence_files) ||
...
export function validateWindowsEvidence(
  ...
  if (official)
    fail(
      'official mode requires live final-byte and Authenticode verification',
    );
...
async function inspectAuthenticode(path) {
  if (process.platform !== 'win32')
    fail('official Authenticode verification requires Windows');
  const script = fileURLToPath(
    new URL('./windows/read-authenticode-evidence.ps1', import.meta.url),
  );
...
export async function validateOfficialWindowsEvidence(
  ...
  const [manifestBytes, checksums, sbom, provenance, cleanReceipt] =
    await Promise.all([
  ...
  if (
    checksumEntries.some((entry) => !entry) ||
    checksumEntries.length !== evidence.artifacts.length
  )
    fail('SHA256SUMS is malformed or incomplete');
  const checksumMap = new Map(
    checksumEntries.map((entry) => [entry[2], entry[1]]),
  );
  for (const artifact of evidence.artifacts)
    if (checksumMap.get(artifact.name) !== artifact.sha256)
      fail(`SHA256SUMS does not bind ${artifact.kind}`);
  ...
  const provenanceSubjects = new Map(
    (provenance.subjects ?? []).map(({ name, sha256: digest }) => [
      name,
      digest,
    ]),
  );
  if (
    ...
    evidence.artifacts.some(
      (artifact) => provenanceSubjects.get(artifact.name) !== artifact.sha256,
    )
  )
  ...
  if (
    ...
    cleanReceipt.candidate_manifest_sha256 !==
      createHash('sha256').update(manifestBytes).digest('hex') ||
    cleanReceipt.windows?.architecture !== 'x86_64' ||
    ['edition', 'build', 'webview2_version'].some(
      (key) =>
        typeof cleanReceipt.windows?.[key] !== 'string' ||
        cleanReceipt.windows[key].length === 0 ||
        cleanReceipt.windows[key] === 'REQUIRED',
    ) ||
  ...
  if (
    signatureEvidence.schema_version !== 1 ||
    !Array.isArray(signatureEvidence.artifacts) ||
    signatureEvidence.artifacts.length !== 2
  )
  ...
  const expectedTargets = [
    {
      ...
      path: join(
        root,
        evidence.artifacts.find(({ kind }) => kind === 'nsis').name,
      ),
    ...
    },
    {
      ...
      digest: evidence.portable_inventory.find(
        ({ relative_path }) => relative_path === 'Glitchpad.exe',
      ).sha256,
    },
  ];
  for (const target of expectedTargets) {
    ...
    if (
      ...
      fail(
        `Authenticode trust, publisher, or timestamp is invalid for ${target.kind}`,
      );
    const recorded = signatureEvidence.artifacts.find(
    ...
    if (
      ...
      fail(
        `signature evidence does not match live verification for ${target.kind}`,
      );
    ...
    if (manifestArtifact.signature_sha256 !== target.digest)
      fail(
        `official signature evidence does not bind ${manifestArtifact.kind}`,
      );
  }
  return true;
}
...
async function main() {
  ...
  if (evidenceIndex >= 0) {
    ...
    if (process.argv.includes('--official')) {
      ...
      await validateOfficialWindowsEvidence(evidence, contract, {
        artifactRoot,
      });
    } else {
  ...
  console.log(
    `Validated ${result.capabilityCount} Windows extensions and ${result.artifactCount} artifact contracts.`,
  );
}
