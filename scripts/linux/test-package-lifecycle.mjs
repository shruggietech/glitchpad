...
import { classifyPackageSize, validateCleanEnvironmentReceipt, validateLinuxEvidence } from '../check-linux-package.mjs';

const execFileAsync = promisify(execFile);
const delay = (milliseconds) => new Promise((resolvePromise) => setTimeout(resolvePromise, milliseconds));
const resultKeys = [
  'artifact_integrity', 'install_or_extract', 'desktop_registration', 'mime_registration',
  'launch', 'startup_delivery', 'running_instance_delivery', 'read', 'edit', 'save',
  'metadata', 'recovery', 'remove', 'registration_cleanup', 'document_preservation',
];
const manualKeys = [
  'dialog', 'drag_drop', 'save_as', 'print', 'keyboard', 'focus', 'text_scale',
...
export function validateLifecycleOptions({ packageForm, release }) {
  if (!['appimage', 'deb'].includes(packageForm)) throw new Error('unsupported package form');
  if (!['22.04', '24.04'].includes(release)) throw new Error('unsupported Ubuntu release');
  return true;
}

export function percentile95(samples) {
  if (!Array.isArray(samples) || samples.length < 1 || samples.some((value) => !Number.isFinite(value) || value <= 0))
...
function parseArguments(arguments_) {
  const result = {};
  const names = new Map([
    ['--package', 'packagePath'], ['--manifest', 'manifestPath'], ['--receipt', 'receiptPath'],
    ['--package-form', 'packageForm'], ['--release', 'release'],
  ]);
...
async function command(program, arguments_, options = {}) {
  try {
    return await execFileAsync(program, arguments_, { encoding: 'utf8', maxBuffer: 8 * 1024 * 1024, ...options });
  } catch (error) {
...
async function waitForMarker(probeRoot, previousCount, timeoutMilliseconds = 10_000) {
  ...
  while (performance.now() - started < timeoutMilliseconds) {
    ...
    if (names.includes('shell-ready.marker') && deliveries.length === previousCount + 1) return;
    if (deliveries.length > previousCount + 1) throw new Error('delivery_acknowledgement_duplicate');
    await delay(50);
  }
...
async function stop(child) {
  ...
  if (!exitedAfterKill && child.exitCode === null) throw new Error('application_process_did_not_exit');
}

async function requireSuccessfulExit(child, timeoutMilliseconds = 5_000) {
  if (child.exitCode !== null) {
    if (child.exitCode !== 0) throw new Error(`running_instance_delivery_failed:${child.exitCode}`);
    return;
  }
  let timeout;
  try {
    const code = await Promise.race([
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(() => reject(new Error('running_instance_process_did_not_exit')), timeoutMilliseconds);
      }),
    ]);
...
async function executableFor(packagePath, packageForm, root) {
  if (packageForm === 'appimage') {
    ...
    return { executable: join(packageRoot, 'AppRun'), packageRoot, uninstall: async () => rm(packageRoot, { recursive: true, force: true }) };
  }
  await command('dpkg', ['--install', packagePath]);
  return { executable: '/usr/bin/glitchpad-host', packageRoot: '', uninstall: async () => command('dpkg', ['--remove', 'glitchpad']) };
}
...
async function prepareDesktopRegistration(installed, packageForm, xdgData) {
  const desktopSource = packageForm === 'appimage'
    ? join(installed.packageRoot, 'usr', 'share', 'applications', 'Glitchpad.desktop')
    : '/usr/share/applications/Glitchpad.desktop';
  const mimeSource = packageForm === 'appimage'
    ? join(installed.packageRoot, 'usr', 'share', 'mime', 'packages', 'glitchpad.xml')
  ...
  const mimeLine = desktop.split(/\r?\n/u).find((line) => line.startsWith('MimeType='));
  const registeredMimeTypes = mimeLine?.slice('MimeType='.length).split(';').filter(Boolean) ?? [];
  if (!desktop.includes('Exec=') || !desktop.includes(' %F') || !registeredMimeTypes.includes('text/markdown') || !registeredMimeTypes.includes('text/vnd.mermaid'))
  ...
  if (packageForm === 'appimage') {
    ...
    await Promise.all([cp(desktopSource, desktopPath), cp(mimeSource, mimePath)]);
    await command('update-desktop-database', [join(xdgData, 'applications')]);
    await command('update-mime-database', [join(xdgData, 'mime')]);
  }
  const mimeDatabaseRoot = packageForm === 'appimage' ? join(xdgData, 'mime') : '/usr/share/mime';
  const globs = await readFile(join(mimeDatabaseRoot, 'globs2'), 'utf8');
  if (!globs.includes('80:text/vnd.mermaid:*.mmd') || !globs.includes('80:application/x-typescript:*.ts'))
    throw new Error('mime_registration_failed');
  return async () => {
    if (packageForm === 'appimage') {
      await Promise.all([rm(desktopPath, { force: true }), rm(mimePath, { force: true })]);
      await command('update-desktop-database', [join(xdgData, 'applications')]);
      await command('update-mime-database', [join(xdgData, 'mime')]);
    }
...
async function main() {
  if (process.platform !== 'linux') throw new Error('linux_lifecycle_requires_linux');
  ...
  const [manifestBytes, contract, packageBytes] = await Promise.all([
    ...
    readFile(new URL('../../packaging/linux/package-contract.json', import.meta.url), 'utf8').then(JSON.parse),
    readFile(packagePath),
  ]);
  const manifest = JSON.parse(manifestBytes.toString('utf8'));
  validateLinuxEvidence(manifest, contract);
  const expectedArtifact = manifest.artifacts.find(({ kind }) => kind === options.packageForm);
  if (!expectedArtifact || createHash('sha256').update(packageBytes).digest('hex') !== expectedArtifact.sha256)
    throw new Error('artifact_integrity_mismatch');

  const root = await mkdtemp(join(tmpdir(), `glitchpad-linux-${options.packageForm}-`));
  const fixture = join(root, 'document.md');
  const probeRoot = join(root, 'probes');
  const xdgData = join(root, 'xdg-data');
  const fixtureBytes = Buffer.from('# Glitchpad Linux lifecycle\n\nNative package fixture.\n');
  let child;
  let installed;
  ...
  try {
    ...
    verifyRegistrationCleanup = await prepareDesktopRegistration(installed, options.packageForm, xdgData);
    process.stderr.write(`lifecycle:${options.release}:${options.packageForm}:installed\n`);
    const environment = {
      ...process.env,
    ...
    for (let sample = 0; sample < 5; sample += 1) {
      ...
      const started = performance.now();
      child = spawn(installed.executable, [fixture], { env: environment, stdio: 'ignore' });
      await waitForMarker(probeRoot, 0);
      startupSamplesMs.push(Math.ceil(performance.now() - started));
      process.stderr.write(`lifecycle:${options.release}:${options.packageForm}:startup-${sample + 1}\n`);
      if (sample === 0) {
        const second = spawn(installed.executable, [fixture], { env: environment, stdio: 'ignore' });
        await waitForMarker(probeRoot, 1);
        await requireSuccessfulExit(second);
        process.stderr.write(`lifecycle:${options.release}:${options.packageForm}:running-instance-delivery\n`);
      }
      await stop(child);
      child = undefined;
    }
    ...
    const receipt = buildCleanEnvironmentReceipt({
      ...
      webkitgtkVersion: (await command('dpkg-query', ['--show', '--showformat=${Version}', 'libwebkit2gtk-4.1-0'])).stdout.trim(),
      startupSamplesMs,
      startupClassification: p95 <= 1_500 ? 'pass' : p95 <= 2_500 ? 'warning' : 'failure',
      artifactSizeClassification: classifyPackageSize(packageBytes.length, contract.size_budget),
    });
    ...
    process.stderr.write(`lifecycle:${options.release}:${options.packageForm}:complete\n`);
  } finally {
    await stop(child);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  main().catch((error) => {
    process.stderr.write(`${error instanceof Error ? error.message : 'linux_lifecycle_failed'}\n`);
    process.exitCode = 1;
  });
}
