...
try {
    ...
    $manifestPath = & (Join-Path $PSScriptRoot 'assemble-package.ps1') -Version '0.1.3' -Executable $executable -Installer $installer -OutputRoot $output -SourceCommit ('a' * 40) -WorkflowIdentity 'local-contract-test' -RepositoryRoot $repositoryRoot
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $output 'glitchpad-0.1.3-windows-x86_64.zip') -PathType Leaf)) {
        throw 'Portable ZIP was not created.'
    }
    try {
        & (Join-Path $PSScriptRoot 'assemble-package.ps1') -Version '0.1.3' -Executable $executable -Installer $installer -OutputRoot $output -SourceCommit ('a' * 40) -WorkflowIdentity 'overwrite-test' -RepositoryRoot $repositoryRoot | Out-Null
