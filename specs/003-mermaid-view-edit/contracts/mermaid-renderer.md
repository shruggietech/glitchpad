...
The renderer context returns a `DiagramRenderResult` and no side effects. It cannot request a file, save, open a link, access the native bridge, read storage, fetch a resource, or create a child window.
...
flowchart TB
  ...
  valid -->|Yes| render["Render in restricted context"]
  render --> sanitize["Apply final SVG allowlist"]
  sanitize --> output{"Output and time limits pass?"}
  output -->|No| limited
...
7. A restricted context that exceeds five seconds or becomes unhealthy is disposed and replaced before later work.
