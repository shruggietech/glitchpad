...
**Decision**: Integrate the locked Mermaid 11.x package through its public `parse` and `render` functions. Do not bundle Chromium, invoke a CLI, or use a remote rendering service.
...
**Primary evidence**: [Mermaid API usage and syntax validation](https://mermaid.js.org/config/usage), [Mermaid API interface](https://mermaid.js.org/config/setup/mermaid/interfaces/Mermaid.html)

## R3. Combine strict Mermaid configuration with an application-owned security boundary

**Decision**: Initialize Mermaid with strict security, immutable secure configuration keys, no start-on-load behavior, bounded text and edge counts, no callbacks, and deterministic identifiers. Render in a restricted context without native or network authority, then sanitize generated SVG through a Glitchpad allowlist before insertion.
...
**Alternatives considered**: Loose and antiscript modes permit active labels or clicks and are prohibited. Mermaid's beta sandbox mode alone does not satisfy output validation or native-boundary requirements. Trusting only built-in sanitization gives the application no stable allowlist contract.

**Primary evidence**: [Mermaid security levels](https://mermaid.js.org/config/schema-docs/config-properties-securitylevel.html), [Mermaid secure configuration](https://mermaid.js.org/config/schema-docs/config-properties-secure.html), [Mermaid security guidance](https://mermaid.js.org/community/security)
...
**Decision**: Start preview validation 300 milliseconds after the newest edit, allow only one in-flight request per document and two app-wide, key every request by exact source revision, and commit only a result matching the newest revision. Enforce 1 MiB standalone render input, 256 KiB per embedded block, 2,000 edges, 8 MiB sanitized output, and a 5 second timeout.

**Rationale**: The chosen delay stays inside the 250–500 ms acceptance window and avoids rendering each keystroke. Revision keys solve out-of-order completion deterministically. Explicit limits prevent large diagrams and documents containing many blocks from monopolizing memory or interaction time while retaining source editing.
