...
**Branch**: `codex/015-mermaid-view-edit` | **Date**: 2026-09-03 | **Spec**: [spec.md](spec.md)

**Input**: Feature specification from `specs/015-mermaid-view-edit/spec.md`
...
**Primary Dependencies**: Tauri 2.x, React 19.x, CodeMirror 6, unified/remark/rehype, `rehype-sanitize`, Mermaid 11.17.2, and DOMPurify 3.4.14 under its Apache-2.0 option; exact versions and transitive dependencies are locked by the repository manifests
...
**Constraints**: Compact content-first interface, exact source preservation, offline operation, untrusted input, strict local rendering, no active links or callbacks, no user-file direction rewriting, one render per document, bounded app-wide concurrency, Apache-2.0-compatible distribution, no export or cloud surface, and cooperative rather than preemptive engine timeout inside a single WebView
...
**Structure Decision**: Implement Mermaid as one internal domain adapter shared by standalone documents and Markdown block projections. The standalone surface composes the existing text editor and source lifecycle. Markdown extracts bounded fenced-block placeholders and submits them to the same adapter without transferring ownership of the surrounding document. The adapter imports no host or Tauri APIs. Generated SVG is sanitized twice, rewritten to collision-free local fragment identifiers, and presented only as an inert static image whose object URL is revoked after use.
...
Render work runs behind a typed adapter that imports no Tauri bridge, storage gateway, external-link gateway, or network API. Application CSP denies connections, objects, frames, media, and base changes. Every request includes owner, source-revision, theme, and generation identities. Timeout, tab suspension, and supersession invalidate the generation; only the newest matching revision can commit output. Mermaid requires a DOM and shares the WebView event loop, so the five-second deadline rejects cooperative late completion but cannot preempt a synchronous third-party engine call. Hard source, edge, output, block, and concurrency preflight bounds are therefore the primary denial-of-service controls.
...
Standalone rendering accepts at most 1 MiB of decoded source, 2,000 edges, 8 MiB of sanitized SVG, a 5 second cooperative deadline, and one in-flight request for the document. Markdown accepts at most 256 KiB per block, 64 blocks and 1 MiB of aggregate Mermaid source per document, subject to the same per-render edge/output/deadline caps and an app-wide scheduler limit of two active requests. Mermaid's module-global configuration is serialized within the one application realm even though two owners may be queued as active, preventing configuration interleaving. Documents above render limits retain source view/edit/search/copy/save through the shared text thresholds.
...
| A render consumes the UI thread or ignores cancellation | Hard preflight limits, patched exact dependency version, one-current-revision commit rule, cooperative deadline rejection, and interaction latency tests; true synchronous preemption is explicitly unavailable in one WebView | Performance and cancellation gate |
