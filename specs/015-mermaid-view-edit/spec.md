...
**Feature Branch**: `[codex/015-mermaid-view-edit]`

**Created**: 2026-09-03

**Status**: Draft

**Input**: User description: "Deliver Mermaid end-to-end by bundling issues #54 through #57: isolated rendering and sanitization, standalone editing, embedded Markdown diagrams, and accessible diagram navigation and metadata."
...
3. **Given** Mermaid source containing links, click callbacks, HTML, scripts, or remote-resource references, **When** the source is rendered, **Then** no script, callback, navigation, native invocation, or network request executes.
...
- A render request that exceeds source, complexity, time, output, memory, or concurrency limits is cancelled; source editing, saving, and copying remain available.
...
- **FR-005**: Mermaid previews MUST refresh 250 to 500 milliseconds after the newest source change, and stale, superseded, hidden, or cancelled render results MUST NOT replace the current preview.
- **FR-006**: A parse failure MUST preserve all source text, keep source editing and saving available, retain the last valid preview for the current document session when one exists, and expose line and column information only when supplied reliably by the parser.
- **FR-007**: Rendered diagrams larger than the viewport MUST support fit-to-view, actual size, zoom from 0.1 through 8.0, reset, and bounded pan through keyboard, pointer, and touch interactions appropriate to the platform.
...
- **FR-010**: Rendered output MUST pass a final element, attribute, and URL allowlist after generation and MUST remain confined to an unprivileged presentation boundary with no native authority.
...
- **FR-013**: Each embedded Mermaid block MUST map to its exact parent source range and revision and MUST render and fail independently so one invalid or over-limit block cannot prevent the remaining Markdown from rendering.
- **FR-014**: Standalone rendering MUST enforce 1 MiB source, 2,000 edges, 8 MiB sanitized output, one in-flight request per owner, and no more than two active application requests; embedded rendering MUST also enforce 256 KiB per block, 64 blocks, and 1 MiB aggregate Mermaid source while leaving source operations available after refusal. Any render result observed after five seconds MUST be rejected, its generation invalidated, and later work allowed to proceed; the single-WebView runtime does not claim synchronous preemption of third-party engine code.
- **FR-015**: The metadata inspector for a Mermaid document MUST report host and text metadata plus detected diagram type, parser status, parser version, accessible-title presence, accessible-description presence, render measurements, and the active resource-limit result when available.
...
- **FR-017**: Mermaid viewing and editing MUST work without an account or network connection on Windows, macOS, Linux, and Android.
...
- **FR-019**: Mermaid rendering failures MUST be classified as malformed source, unsupported syntax, resource limit, cancellation, or internal renderer failure and MUST produce stable, bounded, non-destructive diagnostics without source dumps or stack traces.
...
- **Embedded Mermaid block**: A fenced Mermaid source region owned by a Markdown document, identified by exact source range and revision, with an independent parse and render result.
...
- **Diagram viewport state**: Per-session fit, zoom, bounded pan, search, and focus state that never alters source content.
...
- S015 bundles GitHub issues #54, #55, #56, and #57 as one end-to-end Mermaid capability.
- The completed document detection, text editing, persistence, recovery, and Markdown capabilities from issues #45, #48, #50, #52, and #53 are prerequisites.
- Issue #54 provides the renderer required by #55 and #56; issue #57 depends on the standalone and embedded experiences from #55 and #56.
...
- The bundled parser defines supported diagram syntax. Unsupported syntax never triggers an online fallback or source conversion.
