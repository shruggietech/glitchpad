...
| FR-009 through FR-014 | Core session validation, stable path-free desktop/Android native references, Android startup adapter invocation and bounded materialization, durable-only cross-launch matching, loaded active-session/inspector/presentation application, accepted and newly written recovery-reference integration, 32-entry host bound, category reset and unrelated recovery sentinel tests | Pass |
...
| UI behavior | Preference and diagnostic component accessibility tests, shell integration tests, editor configuration tests, theme propagation tests, Markdown default-mode application, shutdown preference/session flush, combined preference/session load diagnostics | Pass |
