...
- **FR-018**: Clean-environment validation MUST report AppImage launch and Debian install, launch, command-line and file-manager delivery, read, edit, save, Save As, metadata, recovery, print, removal, registration cleanup, and document preservation on Ubuntu 22.04 and Ubuntu 24.04. Candidate-only operations that are not exercised MUST report `not_run_candidate` and MUST block official validation rather than inheriting pass status from separately executed conformance suites.
