...
| Final Windows receipt parsing convergence | Pass | The browser setup advanced both Windows jobs into the installed lifecycle. T035 replaced strict nested member access with explicit `ConvertFrom-Json -AsHashtable` schema extraction; the next run exposed PowerShell cast precedence indexing the dictionary only after converting it to a string, so T036 now parenthesizes every lookup before casting and guards that exact syntax. Both lifecycle scripts parse, all 12 focused Windows tests pass, and the live Windows policy validator succeeds before the next CI run. |
