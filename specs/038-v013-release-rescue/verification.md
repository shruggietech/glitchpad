...
| Final Windows receipt parsing convergence | Pass | The browser setup advanced both Windows jobs into the installed lifecycle. T035 replaced strict nested member access with explicit `ConvertFrom-Json -AsHashtable` schema extraction; subsequent runs exposed runner-version-sensitive ordered-dictionary index binding and nested dictionary pipeline conversion. T036 now resolves every complete receipt path internally by strict ordinal enumeration and returns only final scalar values. Both lifecycle scripts parse, all 12 focused Windows tests pass, and the live Windows policy validator succeeds before the next CI run. |
