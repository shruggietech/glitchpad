...
| Cross-artifact analysis | Pass | The initial 24 requirements, 12 buildable outcomes, 13 acceptance scenarios, and 25 tasks mapped with zero findings; review and CI convergence appended T026-T045, with all 45 tasks completed and mapped to the affected release requirements |
...
| Final Windows receipt parsing convergence | Pass | The browser setup advanced both Windows jobs into the installed lifecycle. T035 replaced strict nested member access with explicit schema extraction; subsequent runs showed that GitHub's PowerShell binding could not portably consume either ordered-hashtable or projected-object JSON representations. T036 now reads exact manifest and receipt properties through typed `System.Text.Json.JsonDocument` APIs and disposes each document deterministically. Both lifecycle scripts parse, all 12 focused Windows tests pass, and the live Windows policy validator succeeds before the next CI run. |
| Packaged UI Automation convergence | Pass | Typed receipt parsing advanced both Windows jobs into the real installed lifecycle, where WebView UI Automation advertised Invoke support but returned an unrecognized-error exception. T037 preserves semantic Invoke as the first choice and falls back to the existing focused clickable-point mouse path for both buttons and menu commands; the policy suite rejects removal of the caught-invocation fallback. |
| Packaged click-geometry convergence | Pass | The next exact-package run reached the caught-invocation fallback, where the WebView provider did not expose a clickable point. T038 preserves the provider point when available and otherwise clicks the center of a visible, nonzero bounding rectangle; the policy suite rejects removal of that geometry fallback. |
| Packaged command-target convergence | Pass | The next exact-package run showed that a generic same-name lookup could select the zero-sized `Edit source` text descendant. T039 requires menu commands to resolve the actionable named Button control before semantic invocation or click fallback; the policy suite rejects reintroducing generic element selection. |
| Packaged accessibility-tree convergence | Pass | The next exact-package run showed that WebView does not classify the actionable `Edit source` ancestor as a Button. T040 begins at the named descendant and walks the control-view ancestry to the first visible, nonzero Invoke-capable element before activation; package policy rejects removal of that tree-walk target resolution. |
| Packaged menu-activation convergence | Pass | The next exact-package run showed that the WebView provider can apply a menu command before its Invoke call throws, leaving the previously valid element with zero geometry. T041 activates the already validated menu target through the real click path directly while ordinary buttons retain semantic Invoke-first behavior; package policy rejects ambiguous menu invocation. |
| Packaged save convergence | Pass | The next exact-package run entered source editing and issued Ctrl+S, but the WebView accessibility tree did not expose the transient `saved durably` status. T042 polls the actual fixture until its UTF-8 bytes equal the intended saved text before previewing and retains the final exact-content check; package policy rejects removal of byte-level durability evidence. |
| Packaged Save-command convergence | Pass | Byte-level polling showed that simulated Ctrl+S did not reach the packaged save handler even though the editor content changed. T043 drives the application's explicit Save menu command through the validated real-click path, then requires exact persisted bytes and successful preview output; keyboard command behavior remains covered by the existing application and release-evidence tests. |
| Shortcut-bearing command convergence | Pass | Both exact-package runs showed that WebView exposes the Save menu item's combined label and `Ctrl+S` shortcut rather than a standalone actionable `Save` name. T044 matches only the exact label-plus-shortcut accessible name, then retains the validated ancestor and real-click path; package policy rejects removal of shortcut-aware targeting. |
| Native save-revision convergence | Pass | The shortcut-aware run reached the real Save command but exact bytes remained unchanged. Code tracing found that the native source host stayed at its initial session revision while the desktop gateway submitted the edited revision, causing every practical edited save to fail closed as stale. T045 adds an ordered revision-synchronization IPC before save, rejects revision regression in the host, and covers the two-call contract and native stale guard. |
