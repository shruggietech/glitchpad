...
| Final Windows receipt parsing convergence | Pass | The browser setup advanced both Windows jobs into the installed lifecycle. T035 replaced strict nested member access with explicit schema extraction; subsequent runs showed that GitHub's PowerShell binding could not portably consume its own ordered-hashtable JSON representation. T036 now parses ordinary JSON objects, resolves each complete receipt path through ordinal `PSObject.Properties` comparison, and returns only final scalar values. Both lifecycle scripts parse, all 12 focused Windows tests pass, and the live Windows policy validator succeeds before the next CI run. |
